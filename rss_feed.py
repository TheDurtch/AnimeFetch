            ...
            if os.path.isfile(file_path):
                ...
                if subtitle_tracks:
                    ...
                    if result.returncode == 0:
                        os.remove(file_path)
                        print(f"Muxing successful, Onii-chan! {file} has been cleaned up.")
                    else:
                        print(f"Muxing failed, Onii-chan! Moving the original file: {file}.")
                        new_file_path = os.path.join(completed_dir, clean_filename(file))
                        os.rename(file_path, new_file_path)

                else:
                    print("No English subtitle track found, Onii-chan! Moving the file as is.")
                    new_file_path = os.path.join(completed_dir, clean_filename(file))
                    os.rename(file_path, new_file_path)
